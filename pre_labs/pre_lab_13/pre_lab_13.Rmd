...
**Answer:** Here, the r-value is only roughly -0.0599. With an r-value this close to 0, it's clear that per-pupil spending and GDP growth are only barely correlated. Similarly, the t-value is a weak -0.41572, again demonstrating that the relationship between the variables is not statistically relevant. Likewise, the p-value is a sky-high 0.6795 — 13.5 times greater than an accepted p-value of less than .05. This further indicates that any correlation the data may show is likely a statistical aberration. In other words, the relationship between the variables is almost nonexistent, and the correlation is far too random to be statistically relevant.
...
**Answer:** The p-value is roughly 0.074, which isn't all that remarkable as far as a statistical relationship goes — it's not as high as the 0.6795 we saw in one of the previous tasks, of course, but it leaves open a lot of room for the correlation being a statistical aberration. Case in point, it doesn't fall within the p \<.05 rule. Likewise, the t-value of -1.828 isn't really impressive either. All this to say that while there is a difference between the means, it doesn't appear to be statistically relevant because the correlation is too random.
...
**Answer:** Unlike in the previous task, there appears to be a relatively strong statistical relationship between these variables. This is visible in the t-value, which, at 4.2494, shows a statistically relevant relationship between per-pupil spending and voting habits. This conclusion is supported the p-value, which is only 0.0001289 — this shows that it is highly unlikely the relationship is random and further indicates that the correlation is meaningful. Point being, it appears that there is a correlation between states with higher per-pupil spending and states that voted for Biden.
