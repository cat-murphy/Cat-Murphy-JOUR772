...
Link to memo: <https://docs.google.com/document/d/1-DfjY-jJvDedNdl-PNohr5gP9UCz-_G7UZ-kErY5fSs/edit?usp=sharing>
...
*Question: How did the Maryland Senate candidates do in comparison to the national party nominees? What were the county trends?*
...
senate_comparison <- senate_comparison |>
  mutate(
    presidential_winner = case_when(
      pct_harris_walz24-pct_trump_vance24 >= 0.00001 ~ "DEMOCRAT",
      pct_trump_vance24-pct_harris_walz24 >= 0.00001 ~ "REPUBLICAN"),
    senate_winner = case_when (
      pct_alsobrooks-pct_hogan >= 0.00001 ~ "DEMOCRAT",
      pct_hogan-pct_alsobrooks >= 0.00001 ~ "REPUBLICAN"))
...
# calculate difference in margins
((sum(senate_comparison$trump_vance24)/sum(senate_comparison$total_votes24)*100)-(sum(senate_comparison$hogan)/sum(senate_comparison$total_votes_senate)*100))+((sum(senate_comparison$harris_walz24)/sum(senate_comparison$total_votes24)*100)-(sum(senate_comparison$alsobrooks)/sum(senate_comparison$total_votes_senate)*100))

    # answer (rounded): -0.76-point margin
...
# ANSWERS, ISSUES & NOTABLE FINDINGS:
    ...
    # How did Hogan and Alsobrooks do in comparison to Trump and Harris? 

        # First, 78,090 more people voted in the Senate race than did in the presidential race, meaning that a number of people must have left the top of the ticket blank. 

        # Trump performed noticeably worse (-8.29 points) than Hogan: where Trump only received 34.54% of the vote, Hogan received 42.84%.

        # Likewise, while Alsobrooks won the Maryland Senate seat safely, Harris still outperformed her by 7.54 points: Alsobrooks only received 54.64% of the vote, compared to Harris' 62.18%.

        # This suggests that a not insignificant number of people voted for both Hogan and Harris. The prevalence of split-ticket voting is particularly interesting in this case because Hogan is a) a popular former Maryland governor and b) has distanced himself from Trump. This could have made him a more appealing choice to center-right voters who supported conservative policies but not Trump himself. 

        # But this also means that Alsobrooks underperformed Harris by a smaller margin (-0.76-point difference) than Hogan outperformed Trump. In other words, the vote share Alsobrooks lost does not fully account for the vote share Hogan gained. This could suggest that people who voted third-party or write-in at the presidential level may have voted for one of the two major candidates at the Senate level.

    # What were the county trends?

        # This is where things get a little wonky. Maryland's five largest counties — Montgomery, PG, Baltimore County, Baltimore City and Anne Arundel — fully account for the difference between the number of votes in the presidential race and the number of votes in the Senate race. In PG and Montgomery alone, 55,550 fewer people voted for president than voted for Senate. These counties are all Democratic strongholds — Harris still won each safely, with at least 55% of the vote. 

        # In most of these five counties, and in other Democratic counties like Howard and Frederick, Hogan led Trump by a wider margin than his statewide average. In Howard, Anne Arundel, Montgomery and Frederick, for example, Hogan's vote share was more than 10 points higher than Trump's. And in two of these Democratic counties, Anne Arundel (55.19% Harris) and Frederick (52.79% Harris), Harris and Hogan each won a majority of the vote. Again, this suggests a really interesting trend in split-ticket and cross-party voting that would benefit from more research and analysis.
```
