...
**Answer:** Creating a new variable to hold the result here is *critical* because we are asking R to take out or "ignore" certain columns of data. Assuming we don't want to actually delete that data, it's important that we don't modify the original data set but instead create a "copy" of the select data we want under a new variable.
...
```{r}
theatre_seats_15 <- umd_courses |> 
  filter(department == "Theatre") |> 
  filter(seats >= 15)
...
```{r}
and_theatre_seats_15 <- umd_courses |> 
  filter(department == "Theatre" & seats >= 15)
...
**Answer:** Essentially, & means "both" and \| means "either." So, when filtering by classes in the theatre department AND classes with at least 15 seats, R will only spit back the classes that are theatre classes with at least 15 seats. When filtering by classes in the theatre department OR classes with at least 15 seats, R will return with all the classes in the theatre department *in addition to* all the classes in the university with at least 15 seats.
...
*I added count_expenses because I was curious.*

Describe the values in the new `month` column - what do you think is going on there?

**Answer:** From what I'm gathering, the "floor_date" function is taking all of the dates and rounding them down. So, all expenditures dated between June 1, 2022, and June 30, 2022, are rounded down to the "floor" — a.k.a. June 1. Then, all those expenditures rounded down to June 1 are combined to generate the total amount spent in June 2022.
...
**Answer:** We are first telling R to mutate the general_22 data set by create two new columns: "total_votes" and "pct_moore." In the total_votes column, we are asking R to calculate the total number of votes cast in each county by adding together all the data from each of the candidate columns. Then, in the "pct_moore" column, divide the total number of votes Moore received in each county by the total number of votes cast in that county to calculate what percentage of the total vote Moore received in that county.
...
**Answer:** This time we asked R to take the general_22 data and arrange it in descending order of county support for Moore.
...
**Answer:** This is definitely a more useful answer. Moore won the election, so which counties were most supportive of his candidacy — aside from not being surprising — aren't very interesting journalistically. However, which counties *didn't* support him — there might be a story there.
...
-   **str_to_lower** — convert a character column into all lowercase

-   **str_to_title** — convert a character column into all title case (capitalizes the first letter of every word)

-   **str_to_sentence** — convert a character column into sentence case (first letter of the first every sentence is capitalized)
...
**Answer:** Here, the mutate statement is telling R to look at the address column in the maryland_expenses data set and search the data for certain state names — Maryland, California, Washington, etc. Then, when it detects those state names, we're telling R to categorize them in a newly created "state" column in the new data set we just created (maryland_expenses_with_state). So, if R detects "Maryland" in the address, we want R to categorize that address as "MD" in the state column. The end of the statement — .default = NA — is telling R not to categorize the addresses that do not contain any of the states listed. In other words, an "NA" will appear in the "state" column for an address in Connecticut because Connecticut is not one of the state we asked R to string detect for.
...
**Answer:** Logically, yes. I understand *how* R arrived at this answer — it looked at the "state" column we just created in "maryland_expenses_with_state" and calculated the sum of the expenses created in the nine states we asked it to pull from the address column. The "NA" state is just the sum of all the remaining expenses — that is, those associated with payees not addressed in any of the nine specified states.

Now, the data shows that the expenses are overwhelmingly associated with Maryland payees — that makes sense. The second-greatest expense total is associated with the "NA" states — that also makes sense, because that includes payees in 41 states. Now, one would be asking, "Hey, why are Maryland political committees paying so much to organizations in the state of Washington?" But the thing is, they're not. String detect is only looking for "Washington" — meaning addresses in Washington state are listed with addresses in Washington, D.C. In other words, our str_detect functions would need to be modified to separate these in our data set.
...
1.  How many political committee expenses were over \$1,000, listed as "other expenses" in the expense_category column and listed as "other" in the expense_purpose column?

2.  How many classes focus on social justice issues? Which departments are they in? What level are they taught at?

3.  How many 100-level classes have more than 250 seats? How many have fewer than 15?
