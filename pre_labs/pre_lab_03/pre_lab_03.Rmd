...
**Answer:** There are parsing issues with some of the ZIP codes because some of the data are character strings, three digits, etc.
...
**Answer:** No more parsing issues. But instead of having 4 chr columns and 4 dbl columns, R now believes there are 5 chr columns and 3 dbl columns because the some of the vendor ZIP entries now included in the guess_max are characters strings.
...
**Answer:** There are 8,458 zip codes with fewer than five characters — they were likely entered incorrectly.
...
-   There are some entries that aren't categorized (2010, 2011, 2013, 2017

-   There are 50 entries in 2013 that are categorized as "L" instead of "Loan"

-   There is a "contract" entry in 2017
...
-   There are two grants for under \$100, and another under \$200. It seems strange that someone would go through all the paperwork to get a \$60 loan — is this correct?
...
**Answer:** All lowercase, all spaces turned to underscores
...
**Answer:** 25 duplicates (50 entries total)
...
**Answer:** The new zip5 column is standardized — +4s have been cut out
...
... But then I realized that some of the grants were from multiple agencies (e.g. Commerce and Maryland Tourism Board). So, I rewrote my code to str_detect for sources that *include* "MARYLAND TOURISM BOARD," and the results are far more comprehensive:

```{r}
tourism_board_grants_loans <- cleaned_md_grants_loans |>
  filter(
    str_detect(source, "MARYLAND TOURISM BOARD")
  )
```

```{r}
tourism_board_grants_loans |>
  group_by(zip5) |>
  summarise(
    count=n(),
    total_amount=sum(amount)
...
Of course, the ZIP code that received the most money — Baltimore — is the same in both calculations, but the total amount is far greater. And to answer the question, yes, Baltimore receiving the most money makes sense; it's the pride of Maryland *and* the state's largest city.
