...
**Answer: Generally, yes, but the "total" column in the 2022 data is written as "Total" instead of "TOTAL"**
...
**Answer: Originally, there were 11 — because of the "Total" vs. "TOTAL" problem — but because we renamed that column in the 2022 data, there are now the correct number of rows (10).**
...
**Answer: Population data and race data for those 18+, broken down by county with corresponding geocodes.**
...
**Answer: PG has the highest % of voting-age Black people at 60.9%. Of all the voters in PG, 77.1% are Democrats.**

```{r}
maryland_population_with_voters_and_demographics |>
  mutate(
    pct_black_18_plus = (pop_black/pop_18_over)*100, 
    pct_dems = (DEM/TOTAL)*100) |> 
...
```{r}
assessment_data <- read_csv('data/assessment.csv')

maryland_zcta_with_assessment <- maryland_zcta |> 
  left_join(assessment_data, join_by(ZCTA5N == zipcode))
