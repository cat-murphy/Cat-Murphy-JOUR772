...
md24 <- md24 |>
  mutate(
    total_votes = harris_walz + trump_vance + write_ins + third_party,
    pct_harris_walz = harris_walz/total_votes*100,
    pct_trump_vance = trump_vance/total_votes*100,
    pct_write_ins = write_ins/total_votes*100,
    pct_third_party = third_party/total_votes*100
  )
...
md20 <- md20 |>
  mutate(
    total_votes = trump_pence + biden_harris + write_ins + third_party,
    pct_trump_pence = trump_pence/total_votes*100,
    pct_biden_harris = biden_harris/total_votes*100,
    pct_write_ins = write_ins/total_votes*100,
    pct_third_party = third_party/total_votes*100
  )
...
md20_registered_voters <- md20_registered_voters |>
  ...
  rename(total_registered=total) |>
  rename(registered_dem=dem) |>
  rename(registered_rep=rep) |>
  rename(registered_bar=bar) |>
  rename(registered_grn=grn) |>
  rename(registered_lib=lib) |>
  rename(registered_wcp=wcp) |>
  rename(registered_oth=oth) |>
  rename(registered_una=una)
...
md24_registered_voters <- md24_registered_voters |>
  ...
  rename(registered_oth=oth) |>
  rename(registered_una=una) |>
  rename(registered_nlm=nlm) |>
  rename(registered_grn=grn)
```

```{r}
# join data
md24_combined <- md_population |>
...
md24_combined_totals <- md24_combined |>
  select(-harris_walz, -trump_vance, -write_ins, -third_party, -pct_harris_walz, -pct_trump_vance, -pct_write_ins, -pct_third_party, -registered_dem, -registered_rep, -registered_lib, -registered_oth, -registered_una, -registered_nlm, -registered_grn)
```
