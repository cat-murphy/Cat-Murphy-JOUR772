...
p20 |> 
  ...
  rename(population2020=value) -> p20

p10 |> 
  select(GEOID, NAME, value) |> 
  rename(population2010=value) -> p10
...
```{r}
alldata |> 
  mutate(change = ((population2020-population2010)/population2010)*100) |>
  arrange(desc(change))
```
...
**Answer: Howard County has the highest median household (\$140,971)**
...
**Answer: I would change it to something along the lines of "Estimated median household income by Maryland county, 2018-2022"**
...
As you can see, some of the error bars are quite wide. Some are narrow. But if the bars overlap, it means the difference between the two counties is within the margin of error, and the differences aren't statistically significant.![](http://127.0.0.1:23531/chunk_output/s/3D252F92/cd7ncohi95no2/000012.png)

Is the difference between Calvert and Montgomery significant?

**Answer: No — the difference is within the margin of error.**

Is the difference between Howard and everyone else significant?

**Answer: Yes. Howard County's minimum estimated median income is at least \~\$6,000 higher than the maximum estimated median income of the next closest county.**
...
md12 |>
  ...
  mutate(
    difference = (estimate2022-estimate2012),
    change = ((estimate2022-estimate2012)/estimate2012)*100
    ) |>
...
**Add your answer here: I'm taking this to mean which counties saw the greatest percent increase in income between 2012 and 2022. In that respect, Baltimore City saw the greatest increase (43%), followed by Garrett County (42.1%) and Allegany County (41.3%). These are not the counties that saw the greatest absolute increase in income, though. Here, Calvert County saw the greatest absolute increase (\$35,683), followed by Howard County** **(\$33,150) and Frederick County (\$32,018).**
